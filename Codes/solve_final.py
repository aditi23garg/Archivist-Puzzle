import pandas as pd
import numpy as np
from sentence_transformers import SentenceTransformer
from sklearn.metrics.pairwise import cosine_similarity

print("Loading data...")
bookA_df = pd.read_csv('BookA_test.csv')
bookB_df = pd.read_csv('BookB_test.csv')

print("Loading model...")
model = SentenceTransformer('all-MiniLM-L6-v2')
print("Ready!")

def get_tail(text, n=200):
    return " ".join(str(text).split()[-n:])

def get_head(text, n=200):
    return " ".join(str(text).split()[:n])

def build_matrix(df, n=200):
    texts = df['text'].fillna("").tolist()
    tails = [get_tail(t, n) for t in texts]
    heads = [get_head(t, n) for t in texts]
    print("  Embedding tails...")
    te = model.encode(tails, show_progress_bar=True)
    print("  Embedding heads...")
    he = model.encode(heads, show_progress_bar=True)
    sim = cosine_similarity(te, he)
    np.fill_diagonal(sim, -999)
    return sim

def confidence_score(trans, i):
    """
    How confident are we about the best next page for page i?
    High confidence = big gap between #1 and #2 choice
    """
    row = trans[i].copy()
    sorted_row = np.sort(row)[::-1]
    return sorted_row[0] - sorted_row[1]

def confident_greedy(trans, start, confidence_threshold=0.01):
    """
    Modified greedy: at each step, only make a choice if we're
    confident enough. If not confident, skip and come back later.
    This avoids locking in wrong choices early.
    """
    n = trans.shape[0]
    visited = [False] * n
    order = [start]
    visited[start] = True
    total = 0.0
    current = start

    for _ in range(n - 1):
        scores = trans[current].copy()
        for vi, v in enumerate(visited):
            if v:
                scores[vi] = -999

        sorted_scores = np.sort(scores)[::-1]
        best_score = sorted_scores[0]
        second_score = sorted_scores[1]
        gap = best_score - second_score

        nxt = int(np.argmax(scores))
        total += trans[current, nxt]
        order.append(nxt)
        visited[nxt] = True
        current = nxt

    return order, total

def path_score(order, trans):
    return sum(trans[order[k], order[k+1]]
               for k in range(len(order)-1))

def two_opt(order, trans, max_iters=500):
    order = list(order)
    n = len(order)
    best_score = path_score(order, trans)
    improved = True
    iters = 0
    while improved and iters < max_iters:
        improved = False
        iters += 1
        for i in range(n - 1):
            for j in range(i + 2, n):
                new_order = (order[:i+1] +
                            order[i+1:j+1][::-1] +
                            order[j+1:])
                new_score = path_score(new_order, trans)
                if new_score > best_score + 1e-9:
                    order = new_order
                    best_score = new_score
                    improved = True
    return order, best_score

def process_book(df, name):
    print(f"\n{'='*40}")
    print(f"Processing {name} ({len(df)} pages)")
    print(f"{'='*40}")
    shuffled_pages = df['page'].tolist()
    n = len(df)

    # Try multiple window sizes and pick best
    best_overall_order = None
    best_overall_score = -np.inf

    for n_words in [150, 200, 250]:
        print(f"\n  Window size: {n_words}")
        trans = build_matrix(df, n_words)

        for s in range(n):
            o, sc = confident_greedy(trans, s)
            if sc > best_overall_score:
                best_overall_score = sc
                best_overall_order = o
                best_trans = trans

    print(f"\n  Best greedy score: {best_overall_score:.4f}")

    best_overall_order, best_overall_score = two_opt(
        best_overall_order, best_trans
    )
    print(f"  Score after 2-opt: {best_overall_score:.4f}")

    rows = [{'original_page': i+1,
             'shuffled_page': shuffled_pages[idx]}
            for i, idx in enumerate(best_overall_order)]
    result = pd.DataFrame(rows)

    assert result['shuffled_page'].nunique() == len(df)
    assert list(result['original_page']) == list(range(1, len(df)+1))
    print("  Validation passed!")

    result.to_csv(f'{name}.csv', index=False)
    print(f"  Saved {name}.csv")
    print(result.head(10).to_string(index=False))

process_book(bookA_df, 'BookA')
process_book(bookB_df, 'BookB')
print("\nDone! Submit BookA.csv and BookB.csv")